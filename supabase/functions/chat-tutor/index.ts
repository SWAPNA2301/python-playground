...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `You are a friendly coding tutor for beginners learning ${context?.language || 'programming'}.

ROLE:
- Help with ${context?.language || 'Python/JavaScript'} concepts
...
CONTEXT:
${context?.level ? `- Current level: ${context.level}` : ''}
${context?.question ? `- Current question: ${context.question}` : ''}
${context?.userCode ? `- User's code: ${context.userCode}` : ''}
